//var sentence = ["Cherish every moment!", "The best time for new beginnings is now!"];
...
window.onload = function() {
    ...
    var Timer = setInterval(function(){
        ...
        if(timeleft == -1) {
            ...
            alert(parseInt(wpm) +" wpm and " + parseInt(accuracy*100) + "% accuracy.");
        }
    },1000);
}
...
$(window).keypress(
    function(event) {
        if(event.which == 32) {
        //     word = document.getElementById("box").value;
        //     len = word.length;
        //     var arr = word.split(" ");
        //     if(check) {
        //         check = false;
        //         spellCheck(word, wordbyword[num]);
        //     }
        //     else{
        //         spellCheck(arr[arrIndex], wordbyword[num]);
        //     }
            
        //     if(len == sentence[count].length) {
        //         //document.write(len);
        //         arrIndex = 1;
        //         len = 0;
        //         count++;
